...
class MaxEntIRL(RobotStateUtils):
    ...
    def max_ent_irl(self, trajectory_features_array, complete_features_array, discount,
        ...
        for i in range(epochs):
            ...
            grad = feature_expectations.reshape(n_features, 1) - expected_svf.reshape(n_features, 1)
            #
            alpha += learning_rate * np.transpose(grad)
            print "alpha is ", alpha

        return complete_features_array.dot(alpha.reshape(n_features, 1)), alpha, policy
