...
def value(policy, n_states, reward, discount,
                    threshold=1e-2):
    """
    ...
    while diff > threshold:
        ...
        for s in range(n_states):
            vs = v[s]
            a = policy[s]
            v[s] = sum((reward[k] + discount * v[k])
                       for k in range(n_states))
            diff = max(diff, abs(vs - v[s]))

    return v

def optimal_value(n_states, n_actions, reward,
...
def find_policy(n_states, n_actions, reward, discount,
    ...
    if v is None:
        v = optimal_value(n_states, n_actions, reward,
                          discount, threshold)
    ...
    def _policy(s):
        return max(range(n_actions),
                   key=lambda a: sum((reward[k] + discount * v[k])
                                     for k in range(n_states)))
