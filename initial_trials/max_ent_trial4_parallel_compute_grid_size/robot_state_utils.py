...
class RobotStateUtils(concurrent.futures.ThreadPoolExecutor):
    def __init__(self, grid_size, discount, terminal_state_val_from_trajectory):
        ...
        self.lin_space_limits_x = np.linspace(-0.5, 0.5, self.grid_size, dtype='float32')
        self.lin_space_limits_y = np.linspace(-0.5, 0.5, self.grid_size, dtype='float32')
        self.lin_space_limits_z = np.linspace(-0.5, 0.5, self.grid_size, dtype='float32')
        ...
        while not converged:
            ...
            for state in self.states.keys():
                ...
                for action in self.action_space.keys():
                    ...
                    newV[state] = reward + self.gamma * V[resulting_state_index]
                # newV = np.array(newV)
...
if __name__ == '__main__':
    ...
    env_obj = RobotStateUtils(3, 0.9, goal)
    ...
    for i in range(10):
        V, policy = env_obj.value_iteration(V, policy, 0.01)
